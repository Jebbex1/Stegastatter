...
from client.gui.options_widget import SteganographyAlgorithmWidget
from client.gui.log_widget import LogWidget


class MainWindow(QMainWindow):
    def __init__(self, title: str, init_size: QSize = QSize(600, 300)):
        super().__init__()
        self.setWindowTitle(title)
        self.setMinimumSize(init_size)
        self.setDockNestingEnabled(True)

        self.control_panel_widget = SteganographyAlgorithmWidget()
        cpanel_dock = QDockWidget("Steganography Control Panel")
        cpanel_dock.setWidget(self.control_panel_widget)
        self.addDockWidget(Qt.DockWidgetArea.BottomDockWidgetArea, cpanel_dock)
