...
def get_decoding_info_from_accepted_blocks(accepted_blocks: np.ndarray, block_shape: tuple[int, int],
    ...
    """
    Extract the decoding info (iv values, conjugation map) from the accepted blocks. This function assumes that the
    ...
    :return: the decoding info with the remaining blocks in the form of: message_block_length, message_remnant_bits_num,
...
def decode_message_from_remaining_blocks(remaining_blocks: np.ndarray, message_block_length: int,
                                         message_remnant_bits_num: int, conjugation_map: list[bool],
                                         block_shape: tuple[int, int]) -> bytes:
    """
    Decodes the message from the image bit plane blocks, using the data from the iv blocks, and the conjugation map.
    ...
    :param message_block_length: the bit length of the message we want to decode in bits
...
def read_message_from_vessel(vessel_blocks: np.ndarray, alpha: float, block_shape: tuple[int, int]) -> bytes:
    """
    Given an array of vessel blocks in the image, extract the iv and conjugation map blocks, and use them to decode the
    ...
    if len(remaining_blocks) < message_block_length:
        raise BPCSDecodeError(f"The message block length iv states that the number of message blocks in the image is "
    ...
    if message_remnant_bits_num > block_shape[0] * block_shape[1]:
        raise BPCSDecodeError(f"The message remnant bits number states that the number of bits that belong to the "
