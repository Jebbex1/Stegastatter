...
def encode(source_image_path: str, message: bytes, output_file_path: str, key: bytes, ecc_block_size: int = 255,
           ecc_symbol_num: int = 16, alpha: float = 0.3, check_capacity=True) -> bytes:
    """
    ...
    img = BPCSImage(source_image_path, as_cgc=True)
    ...
    img.write(output_file_path, arr)
    return token


def decode(source_image_path: str, token: bytes) -> bytes:
    """
...
def check_if_fits_from_bytes(source_image_path: str, message: bytes, ecc_block_size: int = 255,
...
def check_if_fits_from_arbitrary(source_image_path: str, arbitrary_byte_length: int, ecc_block_size: int = 255,
                                 ecc_symbol_num: int = 16, alpha: float = 0.3) -> bool:
    """
